...
#tnew_ = sent_tokenize(b)
for i in range (5):
  tnew[i] = re.sub(r'[^A-Za-z\s\']+', "", tnew[i])
  tnew[i] = '<s> '+tnew[i]+' </s>'
# train_ = tnew_[:133396] # 80% of dataset is train
# test = tnew[133396:] # 20% of dataset is test
test = tnew
print(len(test))
'''
for i in range (len(tnew)):
  tnew[i] = re.sub(r'[^A-Za-z\s\']+', "", tnew[i])
  tnew[i] = '<s> '+tnew[i]+' </s>'
train = tnew[:133396] # 80% of dataset is train
test = tnew[44667:44667+11166] # 20% of dataset is testprint(len(tnew))
'''
...
key,val = fdist1_t.keys(),fdist1_t.values()
dd = {"word":key,"count":val}
df = pd.DataFrame.from_dict(dd)
df.to_csv("bigram_new1.csv")
...
# for i in fdist0.keys():
#   if i in fdist0_t.keys():
#     count_uni+=1

for i in fdist1_t.keys():
  if i in fdist1_t.keys():
    count_bi+=1
print(count_bi)
