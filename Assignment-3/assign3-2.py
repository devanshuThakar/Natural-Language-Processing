...
for i in range (len(tnew)):
  tnew[i] = re.sub(r'[^A-Za-z\s\']+', "", tnew[i])
  tnew[i] = '<s> '+tnew[i]+' </s>'
train = tnew[:44667] # 80% of dataset is train
...
##test dataset using standalone code
...
# for i in range(5):
#   print(generate_ngrams(test[i], 2))
...
# key,val = fdist0.keys(),fdist0.values()
